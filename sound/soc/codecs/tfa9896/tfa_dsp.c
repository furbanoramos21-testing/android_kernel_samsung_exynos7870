...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		...
		is_selective_control +=
			tfa_cont_is_dev_specific_profile(dev, next_profile);
#endif
#if defined(TFA_ACTIVATED_ASYNCHRONOUSLY)
		is_selective_control +=
			tfa_is_selected_dev_to_activate(dev);
#endif
#endif /* TFA_USE_DEVICE_SPECIFIC_CONTROL */
	}

#if defined(TFA_USE_DEVICE_SPECIFIC_CONTROL)
	pr_info("%s: device to be activated selectively = %d\n",
		__func__, is_selective_control);
#endif
...
#endif
#if defined(TFA_ACTIVATED_ASYNCHRONOUSLY)
			if (!tfa_is_selected_dev_to_activate(dev)) {
				err = _tfa_stop(dev);
				continue;
			}
#endif
